...
from escuela.serializers import MateriaSerializer, PersonaSerializer
...
class PersonaList(APIView):
    """
        ...
        """
    def get(self, request, format=None):
        snippets = Persona.objects.all()
        print snippets
        serializer = PersonaSerializer(snippets, many=True)
        return Response(serializer.data)
...
class PersonaDetail(APIView):
    ...
    def put(self, request, pk, format=None):
        snippet = self.get_object(pk)
        serializer = PersonaSerializer(snippet, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
